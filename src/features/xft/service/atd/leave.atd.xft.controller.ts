...
const passOADedupe = new Map<string, NodeJS.Timeout>();
const PASS_OA_DEDUPE_TTL_MS = 2 * 60 * 1000;
const tryLockPassOA = (key: string) => {
  if (!key) return false;
  if (passOADedupe.has(key)) return false;
  const timer = setTimeout(() => passOADedupe.delete(key), PASS_OA_DEDUPE_TTL_MS);
...
export class LeaveEvent {
  ...
  passOA = async () => {
    // Prevent duplicate auto-approve within a short window
    if (!tryLockPassOA(this.task.businessParam)) return false;
