...
export class VehicleRecord extends AbstractContent {
  ...
  static async addRecord(record: ParkingRecordType) {
    await VehicleRecord.addInCarRecord(record);
    await VehicleRecord.addOutCarRecord(record);
  }
  static async addInCarRecord(record: ParkingRecordType) {
    try {
      ...
      // 进厂，插入数据
      const exists = await VehicleRecord.exists({
        where: {
          inRecordId: record.parkingRecordId,
        },
      });
      if (exists) return;
      const count = await VehicleRecord.createQueryBuilder("parkingRecord")
        ...
        .getCount();
      const existingRecord = await VehicleRecord.createQueryBuilder(
        "parkingRecord"
      )
      ...
      if (
        ...
        await VehicleRecord.save(existingRecord);
        return;
      } else {
        const newRecord = VehicleRecord.create({
          ownerId: record.ownerId,
        ...
        await VehicleRecord.save(newRecord);
      }
    } catch (e) {
      logger.error(e);
      logger.error(record);
