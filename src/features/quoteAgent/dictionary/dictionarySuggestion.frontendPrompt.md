...
- 获取提示词说明：`GET /quoteAgent/candidates/clusters/review-prompt`
- 获取候选簇：`GET /quoteAgent/candidates/clusters?status=pending&limit=200`
- 可选按文档过滤：`GET /quoteAgent/candidates/clusters?status=pending&documentId=123&limit=200`
- AI 生成簇级建议：`POST /quoteAgent/candidates/clusters/suggestions/batch`
- 提交批量审核：`POST /quoteAgent/candidates/reviews/batch`
...
- 操作按钮：
  - “刷新候选簇”：调用 `GET /quoteAgent/candidates/clusters`
  - “生成 AI 建议”：调用 `POST /quoteAgent/candidates/clusters/suggestions/batch`
  - “应用已勾选建议”：把选中簇的 `batchOperationsPreview` 转换成 `operations` 提交到 `/quoteAgent/candidates/reviews/batch`
