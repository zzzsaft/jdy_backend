...
async function planOne(documentId: number, options: CliOptions): Promise<BatchResult> {
  const result = await quoteAgentService.planDocumentBlocksWithLlm({
    documentId,
    llmModel: options.model,
    promptVersion: options.promptVersion,
...
async function runPlan(options: CliOptions) {
  ...
  const documents = options.documentId
    ? [await quoteAgentRepository.findDocumentById(options.documentId)].filter(Boolean)
    : await quoteAgentRepository.findDocumentsMissingPlan({
        limit: options.limit,
...
async function itemOne(
  ...
  const result = await quoteAgentService.extractPlannedItemsWithLlm({
    extractionResultId,
    llmModel: options.model,
...
async function runItem(options: CliOptions) {
  await initializeDatabase();
  const extractions = await quoteAgentRepository.findPlannedExtractions({
    limit: options.limit,
    promptVersion: options.promptVersion ?? "v3-plan-item",
    dictionaryVersion: 1,
    llmModel: options.model,
