...
23. 部位、位置和层位必须写入 qualifier，不要为同一基础概念创建部位专用字段。例如侧板/两侧板加热输出 field_name="加热配置"、qualifier.area="side_plate"；模唇加热输出 field_name="加热配置"、qualifier.area="lip"。模唇油加温同时表达“有加热”和“油加温方式”，应拆成加热配置=有、加热方式=油加温，两者都带 area="lip"。
24. A/B/C/D 主机、A/B/C/D 层或 A/B/C/D 区的型号、原料、产量属于同一台前端挤出机配置，按层位聚合为一个 field_name="挤出机型号" 的原文复合值并携带 qualifier.layer；不要把其中的主机产量输出为产品"产量"或"挤出产量"。
25. field_name 只能是业务概念，禁止把数值写入字段名。例如"测温点距内表面6mm"必须输出 field_name="测温点距内表面"、value="6mm"。
26. 图纸状态、参考产品编号和备注必须分别抽取；例如"按原图纸（与190590#一样做）"不能作为一个枚举值，应拆成图纸状态="按原图纸"、参考产品编号="190590"。
27. application 和 plastic material 采用企业口头分类：光学级、弹性体、交联化学发泡等允许作为应用类型；BOPET、BOPE允许作为塑料原料。不要用通用行业分类覆盖原文业务口径。
...
25. 材料/应用/说明混写时必须拆成各自的 split_fields。父 raw_field 的 value/raw_text 保留完整原文用于追溯，但 split_fields 必须覆盖所有有业务意义的片段，后续候选只应来自拆分后的单一属性值：
    ...
    例："CPE（产量：150-200KG左右每小时）" 应拆成 "塑料原料"="CPE"、"产量"="150-200KG左右每小时"，不要输出 "应用类型"="产量150-200KG左右每小时"。
    例："PET（工艺温度：270-280度）" 应拆成 "塑料原料"="PET"、"工艺温度"="270-280度"，不要输出 "应用类型"="工艺温度270-280度"。
26. 部位、位置和层位必须写入 qualifier，不要为同一基础概念创建部位专用字段。例如侧板/两侧板加热输出 field_name="加热配置"、qualifier.area="side_plate"；模唇加热输出 field_name="加热配置"、qualifier.area="lip"。模唇油加温拆成加热配置=有、加热方式=油加温，两者都带 area="lip"。
27. A/B/C/D 主机、A/B/C/D 层或 A/B/C/D 区的型号、原料、产量属于同一台前端挤出机配置，按层位聚合为一个 field_name="挤出机型号" 的原文复合值并携带 qualifier.layer；不要把其中的主机产量输出为产品"产量"或"挤出产量"。
28. field_name 只能是业务概念，禁止把数值写入字段名；例如"测温点距内表面6mm"必须输出 field_name="测温点距内表面"、value="6mm"。图纸状态、参考产品编号和备注必须分别抽取，例如"按原图纸（与190590#一样做）"拆出图纸状态和参考产品编号="190590"。
29. application 和 plastic material 采用企业口头分类：光学级、弹性体、交联化学发泡等允许作为应用类型；BOPET、BOPE允许作为塑料原料。
